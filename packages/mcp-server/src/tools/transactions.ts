...
export function registerTransactionTools(registry: ToolRegistry): void {
  ...
  server.tool(
    ...
    "Create a new transaction to process a payment. Supports three types: WITHDRAW (send funds to a destination), DEPOSIT (pull funds from a source), and SWAP (convert between currencies within the same wallet).",
    {
      type: z
  ...
  server.tool(
    ...
    "Get a single transaction by its unique identifier. Returns full transaction details including status, amounts, fees, and timestamps.",
    {
  ...
  server.tool(
    ...
    "List transactions with optional filters and pagination. Supports filtering by status, type, channel, currency, date range, and reference.",
    {
