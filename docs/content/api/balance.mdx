...
`currencies` is optional — omit it (or call `getBalance()` with no arguments) to get balances for every supported currency:

```typescript
const allBalances = await afriex.balance.getBalance();
```
